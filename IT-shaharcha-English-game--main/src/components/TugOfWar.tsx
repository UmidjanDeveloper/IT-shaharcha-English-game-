...
export default function TugOfWar({
  ...
  // Send postMessage to YouTube iframe without reloading src
  ...
  useEffect(() => {
    return () => {
      if (musicRef.current) {
        try { musicRef.current.src = 'about:blank'; } catch {}
      }
    };
  }, []);
  ...
  // Unmute immediately when battle starts (no src reload!)
  useEffect(() => {
    if (phase === 'battle' && isMusicPlaying) {
      // Try immediately and also after slight delay for slower connections
      ytCommand('unMute');
      ytCommand('setVolume', [85]);
      const t = setTimeout(() => { ytCommand('unMute'); ytCommand('setVolume', [85]); }, 600);
  ...
  // Handle music toggle without src change
  useEffect(() => {
    if (isMusicPlaying && phase === 'battle') {
      ytCommand('unMute');
      ytCommand('setVolume', [85]);
      ytCommand('playVideo');
    } else {
  ...
  return (
    <div className={`w-full max-w-6xl mx-auto px-2 py-2 select-none flex flex-col items-stretch text-center min-h-[520px] transition-all duration-100 ${arenaShake ? 'translate-x-0.5' : ''}`}>
      ...
      {/* Music iframe — src NEVER changes after mount so no buffering delay */}
      {isMusicPlaying && (
        <iframe
          ref={musicRef}
          data-tugofwar="1"
